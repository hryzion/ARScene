...
from networks.physcene.diffusion_scene_layout_ddpm import DiffusionSceneLayout_DDPM
from networks.physcene.optimizer.collision import CollisionOptimizer

from datasets.Threed_front_dataset import ThreeDFrontDatasetPhyScene
...
def load_test_dataset(dataset_dir,dataset_padded_length, num_classes=31):
    return ThreeDFrontDatasetPhyScene(npz_dir=f'{dataset_dir}',split='test',padded_length = dataset_padded_length, num_cate=num_classes)
...
def main():
    ...
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=True, collate_fn=ThreeDFrontDatasetPhyScene.collate_fn_parallel_transformer)
    ...
    physcene = DiffusionSceneLayout_DDPM(
        dataset_num_class+2, feat_extractor, config["network"] # add 2 empty classes
    ).to(device)
    physcene.eval()

    physcene.load_state_dict(torch.load(f"{save_path}", map_location=device))
    cfg = OmegaConf.create(config)
    coll_opt= CollisionOptimizer(cfg.optimizer, normalizer=normalizer)
    physcene.diffusion.diffusion.set_optimizer(coll_opt,None, cfg, None, None)
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(test_loader):
            ...
            room_outer_boxes = batch['outer_boxes'].to(device)
            floor_plan_list = batch['floor_plans']
            floor_plan_list_cuda = []
            for v,f in floor_plan_list:
                floor_plan_list_cuda.append((v.to(device), f.to(device)))
            ...
            infer_room = physcene.sample(
                ...
                floor_plan_centroid=floor_plan_centriods
            )
            ...
            # 这里调用可视化函数，可以传入输入和输出
            visualize_result(decoded_infer, raw_data=decoded_raw, room_name=room_name, save_dir=f'{save_folder}/physcene/topdown_infer/{args.tag}')
