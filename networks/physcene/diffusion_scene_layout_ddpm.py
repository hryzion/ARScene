...
class DiffusionSceneLayout_DDPM(Module):
    ...
    def generate_layout_progressive(self, room_mask, room_outer_box, num_points, point_dim, batch_size=1, text=None, ret_traj=False, ddim=False, clip_denoised=False, batch_seeds=None, device="cpu", keep_empty=False, num_step=100):
        ...
        samples_traj = self.sample(room_mask, num_points, point_dim, batch_size, room_outer_box=room_outer_box, text=text, ret_traj=ret_traj, ddim=ddim, clip_denoised=clip_denoised, batch_seeds=batch_seeds, freq=num_step)
        boxes_traj = {}
    ...
    @torch.no_grad()
    def delete_empty_from_network_samples(self, samples, device="cpu", keep_empty=False, batch_size=1):
        ...
        samples_dict = {
            ...
            "class_labels": samples[:, :, self.bbox_dim:self.bbox_dim+self.class_dim].contiguous(),
            "objectness": samples[:, :, self.bbox_dim+self.class_dim-1:self.bbox_dim+self.class_dim],
        }
        if self.objfeat_dim > 0:
        ...
        boxes = {
            ...
            "translations": torch.zeros(B, 0, self.translation_dim, device=device),
            "sizes": torch.zeros(B, 0, self.size_dim, device=device),
            "angles": torch.zeros(B, 0, self.angle_dim, device=device)
        }
        if self.objfeat_dim > 0:
            boxes["objfeats"] =  torch.zeros(B, 0, self.objfeat_dim, device=device)
    ...
    def delete_empty_boxes(self, samples_dict, device="cpu", keep_empty=False):
        ...
        boxes = {
            ...
            "class_labels": torch.zeros(1, 0, self.n_classes-1, device=device),
...
def train_on_batch(model, optimizer, sample_params, config):
    ...
    loss.backward()
    # Compuite model norm
    grad_norm = clip_grad_norm_(model.parameters(), config.task["train"]["max_grad_norm"])
