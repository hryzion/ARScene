...
from networks.physcene.phyloss.oriented_iou_loss import cal_iou_3d
import random
...
class CollisionOptimizer(Optimizer):
    def __init__(self, cfg: DictConfig, normalizer,  *args, **kwargs) -> None:
        if 'device' in kwargs:
    ...
    def walkable_loss(self,bbox,objectness,floor_plan,floor_plan_centroid,class_labels):
        ...
        for i in trange(len(bbox)):
            ...
            # print(floor_plan_centroid[i].device)
    ...
    def post_process(self, pred_x):
        ...
        denorm_pred_x = self.normalizer.invert_transform_atiss(pred_x)
        bbox_params = {
                        "class_labels": denorm_pred_x[:,:,self.d_bbox:self.d_bbox+self.d_class],
                        "translations": denorm_pred_x[:,:,:3],
                        "sizes": denorm_pred_x[:,:,3:6],
                        "angles":  denorm_pred_x[:,:,6:self.d_bbox],  #TODO
                        "objfeats_32": feat
                    }
