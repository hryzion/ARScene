...
from networks.physcene.optimizer.optimizer import Optimizer
import time
...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, denoise_fn, data, t, condition, condition_cross, clip_denoised: bool, return_pred_xstart: bool):
        ...
        if self.model_mean_type == 'eps':
            ...
            v = model_output
            x_recon = self._predict_start_from_v(data, t, v)
            if clip_denoised:
                x_recon = torch.clamp(x_recon, -1.0, 1.0) 
    ...
    def p_sample(self, denoise_fn, data, t, condition, condition_cross, noise_fn, room_outer_box=None, floor_plan=None, floor_plan_centroid=None, clip_denoised=False, return_pred_xstart=False):
        ...
        """
        model_mean, model_variance, model_log_variance, pred_xstart = self.p_mean_variance(denoise_fn, data=data, t=t, condition=condition, condition_cross=condition_cross, clip_denoised=clip_denoised,
    ...
    def p_sample_loop(self, denoise_fn, shape, device, condition, condition_cross, room_outer_box=None, floor_plan=None, floor_plan_centroid=None,
        ...
        #visualize norm vurve
        #X = torch.cat([translations, sizes, angles, class_labels, objfeats], dim=-1)

        #gradient norm
        # x = [i for i in range(400,len(k_samples))]
        # ytrans = []
        # ysize = []
        # yangle = []
        # yclass = []
        # yfeature = []
        # for i in range(400,len(k_samples)):
        #     norm = torch.norm(k_samples[i][:,:,:3]-k_samples[i-1][:,:,:3])
        #     ytrans.append(norm.item())
        #     norm = torch.norm(k_samples[i][:,:,3:6]-k_samples[i-1][:,:,3:6])
        #     ysize.append(norm.item())
        #     norm = torch.norm(k_samples[i][:,:,6:8]-k_samples[i-1][:,:,6:8])
        #     yangle.append(norm.item())
        #     norm = torch.norm(k_samples[i][:,:,8:8+25]-k_samples[i-1][:,:,8:8+25])
        #     yclass.append(norm.item())
        #     norm = torch.norm(k_samples[i][:,:,8+25:]-k_samples[i-1][:,:,8+25:])
        #     yfeature.append(norm.item())
        # import matplotlib.pyplot as plt
        
        # plt.plot(x,ysize,'s-',color='r',label="size")
        # plt.plot(x,yangle,'s-',color='b',label="angle")
        # plt.plot(x,yclass,'s-',color='y',label="class")

        # plt.plot(x,yfeature,'s-',color='black',label="feature")
        # plt.plot(x,ytrans,'s-',color='g',label="trans")
        # plt.legend()
        # plt.show()
    ...
    def p_sample_loop_trajectory(self, denoise_fn, shape, device, freq, condition, condition_cross, room_outer_box=None,
                                 noise_fn=torch.randn,clip_denoised=True, keep_running=False):
        """
        ...
        for t in reversed(range(0,total_steps)):
            ...
            t_ = torch.empty(shape[0], dtype=torch.int64, device=device).fill_(t)
            img_t, xstart = self.p_sample(denoise_fn=denoise_fn,  data=img_t, t=t_, condition=condition, condition_cross=condition_cross, noise_fn=noise_fn,
    ...
    def set_optimizer(self, optimizer: Optimizer,dataset, cfg, objects_dataset,gapartnet_dataset):
        ...
        self.optimizer.d_class = self.class_dim
        self.optimizer.d_bbox = self.bbox_dim
        self.optimizer.d_shape = self.size_dim
        self.optimizer.dataset = dataset
        self.optimizer.cfg = cfg
        self.optimizer.objects_dataset = objects_dataset
        self.optimizer.gapartnet_dataset = gapartnet_dataset
