...
class CrossAttention(nn.Module):
    def __init__(self, block_idx, embed_dim, cond_dim, num_heads,
        ...
        self.flash = flash_if_available  # optional
        self.attn_l2_norm = attn_l2_norm

    def forward(self, x, kv, attn_bias=None):
        """
        x: (B, L, C)         # query
        kv: (B, L_text, D_text)   # text embedding
        """
        ...
        k = self.k_proj(kv)           # (B, Lk, C)
        v = self.v_proj(kv)           # (B, Lk, C)

        # reshape heads
        q = q.view(B, L, self.num_heads, C // self.num_heads).transpose(1,2)
        k = k.view(B, Lk, self.num_heads, C // self.num_heads).transpose(1,2)
        v = v.view(B, Lk, self.num_heads, C // self.num_heads).transpose(1,2)

        # (B, H, L, Lk)
        attn = (q @ k.transpose(-1, -2)) * self.scale

        if attn_bias is not None:
            attn = attn + attn_bias  # supports (1,1,L,Lk) or (B,1,L,Lk)

        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        # output
        out = attn @ v   # (B, H, L, C/H)
...
class AdaLNSelfAttn(nn.Module):
    ...
    # NOTE: attn_bias is None during inference because kv cache is enabled
    def forward(self, x, cond_BD, attn_bias, key_padding_mask = None):   # C: embed_dim, D: cond_dim
        if self.shared_aln:
...
class AdaLNCrossAttn(nn.Module):
    def __init__(
        self, block_idx, last_drop_p, embed_dim, cond_dim, text_dim,
        shared_aln: bool, norm_layer,
        num_heads, mlp_ratio=4., drop=0., attn_drop=0., drop_path=0.,
        attn_l2_norm=False, flash_if_available=False, fused_if_available=True,
    ):
        super().__init__()
        self.block_idx, self.last_drop_p = block_idx, last_drop_p
        self.C = embed_dim
    ...
    def forward(self, x, cond_BD, cond_cross, attn_bias, key_padding_mask = None):   # C: embed_dim, D: cond_dim
        if self.shared_aln:
            gamma1, gamma2, scale1, scale2, shift1, shift2 = (self.ada_gss + cond_BD).unbind(2) # 116C + B16C =unbind(2)=> 6 B1C
        else:
            gamma1, gamma2, scale1, scale2, shift1, shift2 = self.ada_lin(cond_BD).view(-1, 1, 6, self.C).unbind(2)
        x = x + self.drop_path(self.attn( self.ln_wo_grad(x).mul(scale1.add(1)).add_(shift1), kv=cond_cross, attn_bias=attn_bias ).mul_(gamma1))
        x = x + self.drop_path(self.ffn( self.ln_wo_grad(x).mul(scale2.add(1)).add_(shift2) ).mul(gamma2)) # this mul(gamma2) cannot be in-placed when FusedMLP is used
        return x


class AdaLNDecoderBlock(nn.modules):
    def __init__(
        self, block_idx, last_drop_p, embed_dim, cond_dim, text_dim,
        shared_aln: bool, norm_layer,
        num_heads, mlp_ratio=4., drop=0., attn_drop=0., drop_path=0.,
        ...
        super().__init__()
        self.self_attn_block = AdaLNSelfAttn(block_idx,last_drop_p,embed_dim,cond_dim, text_dim,
            shared_aln, norm_layer,
            num_heads, mlp_ratio, drop, attn_drop, drop_path,
            attn_l2_norm, flash_if_available, fused_if_available
        )
        self.cross_attn_block = AdaLNCrossAttn(block_idx,last_drop_p,embed_dim,cond_dim, text_dim,
            shared_aln, norm_layer,
            num_heads, mlp_ratio, drop, attn_drop, drop_path,
            attn_l2_norm, flash_if_available, fused_if_available
        )
    def forward(self, x, cond_BD, cond_cross, attn_bias, key_padding_mask = None):
        x = self.self_attn_block(x,cond_BD,attn_bias,key_padding_mask)
        x = self.cross_attn_block(x, cond_BD, cond_cross, attn_bias, key_padding_mask)
        return x
...
class RoomLayoutAutoRegressiveNet(nn.Module):
    def __init__(self, 
        ...
        if self.room_mask_condition:
            self.fc_room_mask_f = nn.Linear(self.feature_extractor.output_dim, self.C)
    ...
    def forward(self, x_wo_first, text_desc_c, room_mask_c):
        bg, ed =0, self.L
        B = x_wo_first.shape[0]

        tokenized = self.tokenizer(text_desc_c, return_tensors='pt').to(x_wo_first.device)
        ...
        sos = room_condition
        sos = sos.unsqueeze(1).expand(B, self.first_len, -1) + self.pos_start.expand(B, self.first_l, -1)

        x = torch.cat([sos, x_wo_first],dim=1)
        x += self.lvl_embed(self.lvl_1L[:, :ed].expand(B, -1)) + self.pos_1LC[:, :ed]


        attn_bias = self.attn_bias_for_masking[:, :, :ed, :ed]
        for i, b in enumerate(self.blocks):
            x = b(x=x, cond_BD = room_condition, cond_cross = text_condition_cross, attn_bias = attn_bias)
