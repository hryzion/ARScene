...
from networks.flowscene.flow_scene_layout_fm import FlowSceneLayout_FM
...
def main():
    ...
    if model_type not in ["flowscene"]:
    ...
    save_folder = save_config.get("save_folder", "./pretrained/flowscene/")
    ...
    obj_feat = 64 if dataset_config.get("use_objlat") else 0
    ...
    test_loader = DataLoader(
        ...
        shuffle=True,
        collate_fn=ThreeDFrontDatasetDiffuScene.collate_fn_parallel_transformer,
    )

    feat_extractor = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1).to(device)
    feat_extractor.fc = nn.Identity()

    flowscene = FlowSceneLayout_FM(dataset_num_class + 2, feat_extractor, config["network"]).to(device)
    flowscene.eval()
    flowscene.load_state_dict(torch.load(f"{save_path}", map_location=device))
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(test_loader):
            ...
            denormalized_infer_new = torch.cat([class_part_infer, geom_part_infer], dim=-1)
            ...
            decoded_infer = decode_obj_tokens_with_mask(
                ...
                use_objlat=dataset_config.get("use_objlat", False),
            ...
            for i, scene_json in enumerate(test_scene_jsons):
                json_path = os.path.join(f"{save_folder}/scene/{args.tag}", f"{room_name[i]}_infer.json")
