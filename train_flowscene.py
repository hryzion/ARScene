...
from networks.flowscene.flow_scene_layout_fm import FlowSceneLayout_FM, train_on_batch, validate_on_batch
...
def train_model(
    ...
    if use_wandb:
        WandB.instance().init(
            config,
            model=model,
            project=config["logger"].get("project", "my_FlowScene"),
            name=tag,
            watch=False,
            log_frequency=10,
        )
...
if __name__ == "__main__":
    ...
    train_dataset = ThreeDFrontDatasetDiffuScene(
        npz_dir=dataset_dir,
        split="train",
        padded_length=dataset_padded_length,
        num_cate=len(THREED_FRONT_CATEGORY),
    )
    val_dataset = ThreeDFrontDatasetDiffuScene(
        npz_dir=dataset_dir,
        split="test",
        padded_length=dataset_padded_length,
        num_cate=len(THREED_FRONT_CATEGORY),
    ...
    total_p = count_trainable_params(flowscene) / 1_000_000
